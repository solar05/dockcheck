...
RSpec.describe DockCheck::Inn do
  ...
  it "checks inn correctly" do
    ...
    expect(checker.check(twelve_digit_valid_inn)).to eq({:type=>:inn, :content=>"732897853530", :correct=>true, :error=>""})
    expect(checker.check(twelve_digit_invalid_inn)).to eq({:type=>:inn, :content=>"732897853531", :correct=>false, :error=>""})
  end

  it "throws error when length invalid" do
    expect(checker.check({:type=>:inn, :content=>"8800555"})).to eq({:type=>:inn, :content=>"8800555", :correct=>false, :error=>'Incorrect inn numbers count!'})
  end
end
